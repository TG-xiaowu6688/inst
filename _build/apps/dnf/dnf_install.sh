...
WantedBy=multi-user.target
EOF
cat <<EOF >/lib/systemd/system/comcliproxy1.service
[Unit]
Description=Forward local port 7001 to 5555 (socat) to stafify common login client
After=network.target
[Service]
Type=simple
ExecStart=/usr/bin/socat TCP-LISTEN:7001,bind=127.0.0.1,reuseaddr,fork TCP:127.0.0.1:5555
Restart=on-failure
RestartSec=5
[Install]
WantedBy=multi-user.target
EOF
cat <<EOF >/lib/systemd/system/comcliproxy2.service
[Unit]
Description=Forward local port 7001 to 5555 (socat) to stafify common login client
After=network.target
[Service]
Type=simple
ExecStart=/usr/bin/socat UDP-LISTEN:7001,bind=127.0.0.1,reuseaddr,fork UDP:127.0.0.1:5555
Restart=on-failure
RestartSec=5
[Install]
WantedBy=multi-user.target
...
systemctl enable -q --now mysqlproxy.service comcliproxy1.service comcliproxy2.service
